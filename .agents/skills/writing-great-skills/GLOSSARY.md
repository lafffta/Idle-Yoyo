...
A skill whose `agents/openai.yaml` omits `policy.allow_implicit_invocation` or sets it to `true`, allowing Codex to select it when the task matches its **description**. The human can also invoke it explicitly as `$skill-name`. Pick model-invocation when Codex must reach the skill on its own or another workflow depends on it.

_Avoid_: ability, tool, capability

### Explicit-Only

A skill whose `agents/openai.yaml` sets `policy.allow_implicit_invocation: false`. Codex does not select it merely because a prompt resembles its **description**; the human invokes it as `$skill-name`. The required description remains visible in skill discovery, so keep it short and human-facing.

_Avoid_: procedure, workflow, command

### Description

The skill's machine-readable scope and trigger pointer. Every Codex skill requires one. Its wording drives implicit matching for a **model-invoked** skill and helps humans choose an **explicit-only** skill; invocation policy itself lives in `agents/openai.yaml`.
...
The cost a skill imposes on the agent's context window — especially its always-listed **description**, which spends both tokens and attention. The brake on splitting one workflow into many skills.

_Avoid_: token cost, context bloat

### Cognitive Load

The cost an **explicit-only** skill imposes on the human — what they must hold in their head: which skills exist and when to reach for each (the human is the index). What **model-invocation** removes by being agent-discoverable, and the brake on splitting into more explicit-only skills. Not a cost to minimise: it is the price of human agency. Spend it where human judgement matters; remove it where it does not.

_Avoid_: human index, burden, overhead

### Router Skill

An **explicit-only** skill whose job is to point at other skills — naming each and when to reach for it — so the human has one skill to remember instead of many. The cure for **cognitive load** when explicit-only skills multiply.

_Avoid_: dispatcher, menu, registry, index, router procedure

### Granularity

How finely you divide skills. Finer division spends one of the two loads: more **model-invoked** skills spend **context load** (more descriptions competing for attention); more **explicit-only** skills spend **cognitive load** (more for the human to remember and reach for). Two cuts guide the division. By **invocation**, split off a model-invoked skill where you have a distinct **leading word** to trigger it — a trigger word you actually use in your prompts. By **sequence**, split a run of **steps** where a step's **post-completion steps** need hiding, since isolating it in its own context clears what follows. Beware the reverse: merging sequences exposes each step's post-completion steps to what follows, inviting premature completion.
...
A skill with no **steps** uses just the bottom two rungs — often a legitimately flat peer-set (e.g. every rule of a review on one rung), which is a fine arrangement, not a smell. The hierarchy is independent of invocation: a skill can be model-invoked or explicit-only whether it is all steps, all reference, or both. When a skill has steps, in-file reference that should be disclosed buries them and turns attending to them into a coin-flip — a variance lever, not just a legibility one. Keep the top of the ladder legible; push down it whatever you can.
