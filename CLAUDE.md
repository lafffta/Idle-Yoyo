...
**#72's playtest has happened, and it found the slice wanting.** The manual opening is boring — and was boring while two of the three Tricks were on offer throughout it — while the harness shows the whole ladder exhausted 7m 44s into a 1h 35m run, because `nextTrick` returns `null` once every row is landed. The Auto-Thrower reads as relief from boredom rather than as liberation. The pacing suite was green throughout: a threshold cannot see a ladder empty. What *does* work is the deterministic Attempt itself — a lost Throw was read as the player's own mistake and never as randomness, and nothing about that mechanism changes.

The decision recorded on #72 is **redesign the interaction, do not retune, and do not proceed into Retire.** ADR 0015 makes a landed Trick performable forever — learn once, perform forever, paying Style proportional to the Spin drained rather than a permanent multiplier that would compound without bound — and ADR 0016 rules that Attempts are never automated and that performance income is a bonus for presence, never a penalty for absence. ADR 0004's rejection of "Tricks performed for Style directly" is amended in place: ADR 0002 forbids two rule *sets*, not two *outcomes*, and buying Gear has always been a present-only act that changes earnings. `docs/plans/performable-tricks.md` is the plan. None of it is built yet, and the largest risk is that the tuning harness cannot model a player who partitions Spin across Performances — that is what to find out first.
